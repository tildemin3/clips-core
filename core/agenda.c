...
/**
 * @brief Prints an activation to the specified output.
 * @param theEnv The environment.
 * @param logicalName The logical name of the output.
 * @param theActivation The activation to print.
 */
static void                    PrintActivation(Environment *theEnv,const char *logicalName,Activation *theActivation);

/**
 * @brief Clears the agenda when the clear command is issued.
 * @param theEnv The environment.
 * @param context The context (unused).
 */
static void                    AgendaClearFunction(Environment *theEnv,void *context);

/**
 * @brief Returns the name of the salience evaluation type.
 * @param type The salience evaluation type.
 * @return The name of the salience evaluation type.
 */
static const char             *SalienceEvaluationName(SalienceEvaluationType type);

/**
 * @brief Evaluates the salience of a rule.
 * @param theEnv The environment.
 * @param theRule The rule to evaluate.
 * @return The evaluated salience.
 */
static int                     EvaluateSalience(Environment *theEnv,Defrule *theRule);

/**
 * @brief Reuses or creates a salience group for a given salience value.
 * @param theEnv The environment.
 * @param theRuleModule The rule module.
 * @param salience The salience value.
 * @return A pointer to the salience group.
 */
static struct salienceGroup   *ReuseOrCreateSalienceGroup(Environment *theEnv,struct defruleModule *theRuleModule,int salience);

/**
 * @brief Finds a salience group for a given salience value.
 * @param theRuleModule The rule module.
 * @param salience The salience value.
 * @return A pointer to the salience group, or NULL if not found.
 */
static struct salienceGroup   *FindSalienceGroup(struct defruleModule *theRuleModule,int salience);

/**
 * @brief Removes an activation from its salience group.
 * @param theEnv The environment.
 * @param theActivation The activation to remove.
 * @param theRuleModule The rule module.
 */
static void                    RemoveActivationFromGroup(Environment *theEnv,Activation *theActivation,struct defruleModule *theRuleModule);
